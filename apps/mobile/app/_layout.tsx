...
// 10 — Task 9 had no real screens yet for a redirect to land on. Task 10
// builds those screens (`/` = S02 Portal Select, `/(public)/portal/[role]` =
// S03 Portal Landing), so this wiring belongs here.
//
...
// distinguish a true first-launch signed-out user (-> "/", Portal Select)
...
// subsystem here would be scope creep beyond this task's Files list
// (apps/mobile/app/index.tsx, (public)/portal/[role].tsx, src/config/
// portals.ts, plus this root layout per the Task 9 deferral). The correct,
// minimal choice for Phase-0 is to default `hasOnboarded` to `false`: every
// session is treated as first-launch until a later task adds real
// persistence for this specific flag. This is intentionally conservative —
// worst case a returning signed-out user sees Portal Select once more
// instead of jumping straight to /login, never the other way around — and
// `resolveInitialRoute` already returns "/" for exactly that state, so this
// root layout doesn't redirect away from the screen the user is already on.
...
function RootRedirect() {
  ...
  // literal entry route ("/", Portal Select — where Expo Router always lands
  // first on cold start). Restricting to "/" also structurally guarantees
  // this never fires on "/portal/[role]" (S03) or on any in-app route the
  // user has since navigated to (e.g. a signed-in org user browsing
  // "/(org)/settings" must never get bounced back to "/(org)/dashboard" just
  // because that's their resolved home) — RoleGuard already owns keeping
  ...
  // user — Portal Select is the correct, intentional destination for that
